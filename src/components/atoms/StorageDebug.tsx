import React, { useEffect, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { supabase } from '@/lib/supabaseclient';
import { discoverProjectsFromBuckets } from '@/lib/simpleBucketAccess';

interface DiscoveredProject {
  bucket: string;
  storage_path: string;
  title: string;
  image_categories: Record<string, string[]>;
  thumbnail_url?: string;
}

export const StorageDebug: React.FC = () => {
  const { t } = useTranslation();
  const [buckets, setBuckets] = useState<string[]>([]);
  const [projects, setProjects] = useState<DiscoveredProject[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const run = async () => {
    try {
      setLoading(true);
      setError(null);


      const known = ['challenge-images', 'codelibrary-website', 'devfellowship'];
      setBuckets(known);

      const discovered = await discoverProjectsFromBuckets();
      setProjects(discovered);
    } catch (e: any) {
      console.error('❌ StorageDebug erro:', e);
      setError(e?.message || t('debug.unknownError'));
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    run();
  }, []);

  return (
    <div className="p-4 border border-border rounded-lg bg-card text-sm">
      <div className="flex items-center justify-between mb-2">
        <strong>{t('debug.storageDebug')}</strong>
        <button onClick={run} className="px-2 py-1 text-xs rounded bg-primary text-primary-foreground">{t('debug.reload')}</button>
      </div>
      {loading && <div>{t('debug.loading')}</div>}
      {error && <div className="text-destructive">{t('debug.error')}: {error}</div>}

      <div className="mt-2">
        <div className="font-semibold">{t('debug.buckets')}</div>
        <div>{buckets.join(', ')}</div>
      </div>

      <div className="mt-3">
        <div className="font-semibold">{t('debug.discoveredProjects')} ({projects.length}):</div>
        <ul className="list-disc pl-5">
          {projects.map((p, i) => (
            <li key={i}>
              <div className="font-medium">{p.title} <span className="opacity-60">({p.bucket})</span></div>
              <div className="opacity-70">{p.storage_path}</div>
              <div>{t('debug.sections')} {Object.keys(p.image_categories || {}).join(', ') || '—'}</div>
            </li>
          ))}
        </ul>
      </div>
    </div>
  );
};

